...
function LogoutModal(props) {
    ...
    const handleLogout= async()=>{
        ...
        if(userData){
          await logoutUser(userData?._id).then((res)=>{
...
className='top-0 left-0 absolute w-[100vw] z-[50] h-[100vh] bg-[#00000054] flex justify-center items-center'>
    ...
    className={`bg-[#fff] rounded-[4px] p-5 shadow-xl ${logoutStatus==="logout-loading"?"w-full bg-[#00000014] h-full  justify-center flex items-center text-center":"w-[90%] md:max-w-[500px] max-h-[300px]"}`} >
    <div className='flex flex-row items-center justify-between'>
      <div className=' text-[20px] text-orange font-extrabold '>
      ...
      <div className="mt-4 mb-6 text-[14px]">
        Are you sure you want to <strong>Log out</strong> from FreshMart  
      </div>
      <div className='flex justify-end gap-4 mt-2'>
        ...
        <button className={`rounded-[4px] bg-orange text-[#fff] font-bold text-[12px] py-2 px-5`} onClick={handleLogout}>Log Out</button>
      </div> 
      </>: error?
      <div>
